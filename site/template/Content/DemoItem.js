...
export default class DemoItem extends React.Component {
	...
	getIframe() {
		...
		return (
			<div className="demo-preview-frame">
				<iframe title="base" srr={url} width={width} height={finHeight} frameBorder="0" />
			</div>
		);
	}
	...
	render() {
		...
		return (
			<div className="demo-item" style={{ marginTop: 40 }}>
				<h3 className="demo-title">{title}</h3>
				<div className="demo-desc">{content}</div>
				<div className="demo-preview">{isWide && intoViewport ? children : null}</div>
